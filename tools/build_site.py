...
def attach_verification(pg, summary):
    ...
    for i, e in enumerate(pg.get("entries", [])):
        ...
        for vn in ve.get("nodes", []):
            nodes[str(vn.get("id"))] = {k: vn.get(k) for k in ("verdict", "derivation", "sources", "ref_check", "consulted", "error_in", "book_prints", "correct_form", "derivation_fa") if vn.get(k)}
        ...
        e["verif"] = {k: ve.get(k) for k in ("verdict", "modern_form", "note", "note_fa", "sources", "ref_check", "consulted") if ve.get(k)}
        e["verif"]["nodes"] = nodes
